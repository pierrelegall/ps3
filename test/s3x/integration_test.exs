defmodule S3x.IntegrationTest do
  use ExUnit.Case, async: true

  @test_bucket "test-bucket"
  @test_key "test-file.txt"
  @test_content "Hello, S3x!"

  setup_all do
    # Disable sandbox mode for integration tests
    # Integration tests use HTTP which crosses process boundaries,
    # so we need shared global ETS tables instead of per-process tables
    original_sandbox = Application.get_env(:s3x, :sandbox_mode)
    Application.put_env(:s3x, :sandbox_mode, false)

    on_exit(fn ->
      # Clean up and restore sandbox mode
      try do
        S3x.Storage.clean()
      rescue
        ArgumentError -> :ok
      ...
      Application.put_env(:s3x, :sandbox_mode, original_sandbox)
    end)
  ...
  setup do
    # Reinitialize storage with disabled sandbox mode, then clean
    S3x.Storage.init()
    S3x.Storage.clean()
    :ok
  end
  ...
  describe "ListObjects" do
    ...
    test "lists objects with prefix (not yet supported)" do
      ...
      assert Enum.count(body.contents) >= 1
